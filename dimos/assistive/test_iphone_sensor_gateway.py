...
def test_gateway_ingests_image_imu_and_text() -> None:
    ...
    payload = {
        ...
        "image_b64": _encode_test_image(),
    }

    published = gateway._handle_payload(payload)

    assert set(published) == {"human_input", "imu", "color_image"}
    ...
    assert len(gateway.color_image.values) == 1
    assert isinstance(gateway.color_image.values[0], Image)


def test_gateway_drops_stale_sensor_payloads() -> None:
    gateway = IPhoneSensorGateway()

    stale = time.time() - 20.0

    assert gateway._decode_imu({"ts": stale, "imu": {"accel": {}, "gyro": {}}}, time.time()) is None
    assert gateway._decode_image({"ts": stale, "image_b64": _encode_test_image()}, time.time()) is None
