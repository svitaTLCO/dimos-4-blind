...
export async function POST(req:Request,{params}:{params:{sessionId:string}}){
  try{
    ...
    const u=await fetch(`${BASE}/webscan/sessions/${params.sessionId}/event`,{method:'POST',headers:{"Content-Type":"application/json",...(process.env.DIMOS_WEB_SCAN_TOKEN?{Authorization:`Bearer ${process.env.DIMOS_WEB_SCAN_TOKEN}`}:{})},body,cache:'no-store'})
